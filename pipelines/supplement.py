...
def main():
    ...
    a=np.array(predictions);np.save(OUT/'results/pixel_predictions.npy',a)
    formal=np.load(OUT/'results/predictions.npz')['resnet18|cosine|raw|clean|100|5']
    errors={}
    ...
    (OUT/'results/supplement.json').write_text(json.dumps(result,indent=2));print(json.dumps(result,indent=2))
