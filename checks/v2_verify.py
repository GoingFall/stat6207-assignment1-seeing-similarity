...
def main() -> None:
    checks = []
    sop_lock_path = ROOT / "data_v2/manifests/sop_lock.json"
    ...
    encoding_path = ROOT / "results_v2/sop/encoding.json"
    ...
    retrieval = json.loads((ROOT / "results_v2/sop/retrieval.json").read_text(encoding="utf-8"))
    ...
    neighbours = np.load(ROOT / "results_v2/sop/exact_top1000.npy", mmap_mode="r")
    ...
    inat_lock_path = ROOT / "data_v2/manifests/inat_birds_lock.json"
    ...
    inat_image_lock_path = ROOT / "data_v2/manifests/inat_birds_images_lock.json"
    if inat_image_lock_path.exists():
        ...
        image_manifest = ROOT / image_lock["manifest"]
    ...
    long_tail_index_path = ROOT / "data_v2/manifests/long_tail/index.json"
    ...
    sift_lock_path = ROOT / "data_v2/manifests/sift1m_lock.json"
    sift_result_path = ROOT / "results_v2/ann/sift1m.json"
    ...
    if sift_result_path.exists():
        ...
        audit_path = ROOT / "results_v2/checks/sift1m_exact_audit.json"
    ...
    sift_scales_path = ROOT / "results_v2/ann/sift1m_scales.json"
    ...
    inat_encoding_path = ROOT / "results_v2/inat/encoding.json"
    if inat_encoding_path.exists():
        ...
        for split_name, split in encoding["splits"].items():
            ...
            expected_rows = jsonl(ROOT / metadata_lock["manifests"][split_name]["path"])
            expected = sorted((int(row["image_id"]), int(row["class_id"])) for row in expected_rows)
            image_ids = np.load(ROOT / split["artifacts"]["image_ids"]["path"])
            labels = np.load(ROOT / split["artifacts"]["labels"]["path"])
            vectors = np.load(ROOT / split["artifacts"]["embeddings"]["path"], mmap_mode="r")
    ...
    training_path = ROOT / "results_v2/inat/training.json"
    if training_path.exists():
        ...
        expected_methods = set(json.loads((ROOT / "configs/v2/experiment.json").read_text(encoding="utf-8"))["long_tail"]["methods"])
        ...
        model_path = ROOT / training["balanced_probe"]["model_path"]
    ...
    open_set_path = ROOT / "results_v2/inat/open_set.json"
    if open_set_path.exists():
        ...
        if sha256_file(ROOT / open_set["scores_path"]) != open_set["scores_sha256"]:
    ...
    monitoring_path = ROOT / "results_v2/inat/monitoring.json"
