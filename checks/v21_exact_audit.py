...
def main():
    target=ROOT/'results/v2.1/checks/exact_audit.json'
    if target.exists():raise RuntimeError('Exact revision audit already exists')
    lock=json.loads((ROOT/'data/v2.0/manifests/sift1m_lock.json').read_text())
    base=read_fvecs(resolve(lock['files']['base']['path']))
    q=read_fvecs(resolve(lock['files']['query']['path']))[:256]
    official=read_ivecs(resolve(lock['files']['groundtruth']['path']))[:256]
