...
out=load('results/v2.0/checks/review-2026-09-23.json')
rows=[json.loads(l) for l in (ROOT/'data/v2.0/manifests/inat_birds_images.jsonl').read_text().splitlines()]
...
for name in ('sop/encoding','sop/retrieval','ann/sift1m','inat/training'):
    r=load(f'results/v2.0/{name}.json')
    out['config_hash_links'][name]={'full_config_matches':r.get('config_sha256')==sha('configs/v2.0/experiment.json')}
    for field in ('sop_config','ann_config','exact_config','sift1m_config'):
        if field in r:out['config_hash_links'][name][field+'_matches']=r[field]==load('configs/v2.0/experiment.json')[field.replace('_config','')]
cfg=load('configs/v2.0/experiment.json')['monitoring'];stored=load('results/v2.0/inat/monitoring.json')
ref=np.load(ROOT/'data/v2.0/embeddings/inat_birds/validation_embeddings.npy')
...
for split in cfg['evaluation_splits']:
    x=np.load(ROOT/f'data/v2.0/embeddings/inat_birds/{split}_embeddings.npy');m=drift_metrics(ref,x,proj,cfg['psi_bins'],cfg['rbf_mmd_samples'],cfg['seed'],rbf_gamma=gamma)
...
with zipfile.ZipFile(ROOT/'releases/2.0/Assignment1-2.0.zip') as z:
    ...
    # Names below are read from inside the frozen 2.0 archive, so they keep the pre-migration layout.
...
x=np.load(ROOT/'data/v2.0/embeddings/sop/dinov2_test.npy');idx=faiss.IndexFlatIP(384);idx.add(x)
q=np.random.default_rng(6207).choice(len(x),128,replace=False);_,nn=idx.search(x[q],1001)
nn=np.array([r[r!=i][:1000] for i,r in zip(q,nn)]);saved=np.load(ROOT/'results/v2.0/sop/exact_top1000.npy',mmap_mode='r')[q]
...
(ROOT/'results/v2.0/checks/review-2026-09-23.json').write_text(json.dumps(out,indent=2),encoding='utf-8')
