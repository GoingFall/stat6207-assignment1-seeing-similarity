...
def main():
    ...
    assert (ROOT/'report.pdf').read_bytes()==(ROOT/'site/report.pdf').read_bytes()
    ood=json.loads((OUT/'ood/results.json').read_text());assert len(ood['images'])==20
    for model,items in ood['models'].items():
        assert len(items)==20
        for r in items:
            assert (ROOT/'site/ood'/f"{r['id']}.png").exists()
            assert len(r['nearest'])==5
            for ident in r['nearest']:assert (ROOT/'site/images'/f'{ident}.png').exists()
    meta=json.loads((ROOT/'site/heatmaps/index.json').read_text())
    assert meta['protocol_hash']==(OUT/'protocol.sha256').read_text().strip()
    for record in meta['files']:
        raw=(ROOT/'site/heatmaps'/f"{record['condition']}-{record['metric']}.bin").read_bytes()
        assert len(raw)==500*10*2*7*7 and digest(raw)==record['sha256']
    assert len(meta['files'])==21
    for r in test:
        for condition in p['conditions']:
            with Image.open(ROOT/'site/heatmaps/views'/condition/f"{r['id']}.jpg") as image:assert image.size==(224,224)
    ...
    (OUT/'results/delivery_check.json').write_text(json.dumps(result,indent=2));print(result)
