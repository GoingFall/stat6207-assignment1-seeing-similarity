...
- `report.pdf`: 11-page rubric-aligned report; identical copy in `site/report.pdf`. The active 2.0-dev workspace stores generated report text under `docs/report/`.
- `site/`: static explorer, pairwise ResNet Grad-CAM, UMAP explanations and `ood.html`.
- Source scripts, locked data/results, environment specifications and session transcript support reproduction. In the active 2.0-dev workspace, these are organized under `src/`, `pipelines/`, `reporting/`, `checks/`, `tools/` and `docs/session/`; this does not alter the archived 1.0 payload.
