...
def main():
    p,rows=load_locked();site=ROOT/'site/v1.0';(site/'assets').mkdir(parents=True,exist_ok=True);(site/'images').mkdir(exist_ok=True)
    e=json.loads((R1/'evaluation.json').read_text());audit=json.loads((R1/'label_audit.json').read_text())
    ...
    shutil.copytree(R1/'figures',site/'figures',dirs_exist_ok=True)
    if (ROOT/'report/v1.0/report.pdf').exists():shutil.copy2(ROOT/'report/v1.0/report.pdf',site/'report.pdf')
